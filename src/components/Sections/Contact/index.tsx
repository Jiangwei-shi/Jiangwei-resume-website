...
import BilibiliIcon from '../../Icon/Bilibili';
...
const ContactValueMap: Record<ContactType, ContactValue> = {
  [ContactType.Bilibili]: {Icon: BilibiliIcon, srLabel: 'Bilibili'},
...
const Contact: FC = memo(() => {
  ...
  const {items} = contact;
  const {headerText, description, typeLabels} = messages.contact;
          ...
          <div className="order-1 col-span-1 flex flex-col gap-y-4 md:order-2">
            ...
            <dl className="flex flex-col space-y-4 text-base text-neutral-500 sm:space-y-2">
              {items.map(({type, text, href}) => {
                const {Icon} = ContactValueMap[type];
                const typeSr = typeLabels[type];
                return (
                  <div key={type}>
                    <dt className="sr-only">{typeSr}</dt>
