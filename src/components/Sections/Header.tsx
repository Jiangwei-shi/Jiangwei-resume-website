...
const Header: FC = memo(() => {
  const [currentSection, setCurrentSection] = useState<NavSection | null>(null);

  const intersectionHandler = useCallback((section: SectionId | null) => {
    if (!section) {
      return;
    }
    if (navSections.includes(section as NavSection)) {
...
const DesktopNav: FC<{currentSection: NavSection | null}> = memo(({currentSection}) => {
      ...
      <nav className="flex justify-center gap-x-8">
        {navSections.map(section => (
          <NavItem
            ...
            key={section}
            label={messages.nav[section]}
            section={section}
          />
        ))}
      </nav>
      <LanguageToggle className="absolute right-4 top-1/2 z-[60] -translate-y-1/2" />
    </header>
  );
});

const MobileNav: FC<{currentSection: NavSection | null}> = memo(({currentSection}) => {
  const {messages} = useLocale();
  ...
  return (
    <>
      <button
        aria-label={messages.header.menuButtonAria}
        ...
        <span className="sr-only">{messages.header.openSidebarSr}</span>
      </button>
      <LanguageToggle className="fixed right-2 top-16 z-[60] sm:hidden" />
